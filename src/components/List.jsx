...
class List extends PureComponent {
  ...
  renderTableBody() {
    ...
    return (
      <Fragment>
        <TableRow className={classes.tableTopLine}>
          <TableCell> </TableCell>
        </TableRow>
        {list.map((row, index) => (
          <TableRow
            key={row.id}
            style={{
              background: index % 2 ? '#FFF' : '#E7F0FF',
              transform: deleted.includes(row.id) ? 'translateX(-100px)' : '',
          ...
          >
            <TableCell>
              <Avatar
                ...
                onClick={this.redirectTo(`/info/${row.id}`)}
              />
            </TableCell>
            <TableCell style={{ paddingLeft: 0 }}>
              <div className={classes.nameContainer}>
                <Link
                  ...
                  component={props => (
                    <RouterLink to={`/info/${row.id}`} {...props} />
                  )}
                >
            ...
            <TableCell>
              {row.updatedAt
                ? DateTime.fromISO(row.updatedAt).toRelativeCalendar()
                : ''}
            </TableCell>
            <TableCell>
              {deleted.includes(row.id) ? (
                <IconButton
                  color="default"
                  onClick={this.destroyCancel(row.id)}
                ...
                <IconButton
                  color="default"
                  onClick={this.redirectTo(`/user/account/${row.id}`)}
                  classes={{ root: classes.icon }}
                  component="span"
                >
            ...
            <TableCell align="center">
              {deleted.includes(row.id) ? (
                <p
                  onClick={this.destroyConfirm(row.id)}
