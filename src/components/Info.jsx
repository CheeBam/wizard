  ...
  pageInfo() {
    ...
    const prepareHobbies = (list, object) => {
      return (
        ...
        object.length > 0 &&
        list
          .filter(item => object.includes(item.id))
          .map(item => item.label)
          .join(', ')
      );
    };
    ...
    const formatBirthday = string => {
      return (
        string && DateTime.fromFormat(string, 'dd-MM-yy').toFormat('dd.MM.yy')
      );
    };
              ...
              <Grid key={key} className={classes.partContainer}>
                <Grid item lg={4} md={6} xs={6}>
                  <p className={classes.fontLabel}>
                    ...
                    <IconButton
                      color="default"
                      onClick={this.redirectTo(`/user/${item.link}/${user.id}`)}
                ...
                <Grid container>
                  {item.data.map(
                    (internalItem, internalKey) =>
                      internalItem.hidden || (
                        <Grid key={internalKey} item xs={12}>
                          <Grid container>
                            <Grid item xs={4}>
                              <p className={classes.fontLabel}>
                                {internalItem.label}:
                            ...
                            <Grid item xs={8}>
                              <p className={classes.fontValue}>
                                {internalItem.callback
                                  ? internalItem.callback(
                                      user[internalItem.field]
                                    )
                                  : user[internalItem.field]}
                              </p>
                            </Grid>
